...
type Client struct {
	...
	rect    *proto.Rect   // Прямоугольник с наибольшей площадью
}
...
func (client *Client) handleRequest(req *proto.Request) bool {
	...
	case "add":
		...
		if req.Data == nil {
			...
			var rect proto.Rect
			if err := json.Unmarshal(*req.Data, &rect); err != nil {
				errorMsg = "malformed data field"
			} else {
				x1, errX1 := strconv.ParseFloat(rect.X1, 64)
				y1, errY1 := strconv.ParseFloat(rect.Y1, 64)
				x2, errX2 := strconv.ParseFloat(rect.X2, 64)
				y2, errY2 := strconv.ParseFloat(rect.Y2, 64)
				if errX1 == nil && errY1 == nil && errX2 == nil && errY2 == nil {
					rect.Area = math.Abs((x2-x1)*(y2-y1))
					client.logger.Info("performing addition", "value", strconv.FormatFloat(rect.Area, 'f', 3, 64))
					if client.rect == nil || client.rect.Area <= rect.Area {
						client.rect = &rect
					}
				} else {
					errorMsg = "malformed data field"
	...
	case "find":
		if client.rect == nil {
			client.logger.Error("calculation failed", "reason", "devastation")
			client.respond("failed", "devastation")
		} else {
			client.respond("result", client.rect)
		}
	default:
